...
export const DREGerencialView: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {editing && <ModalOverlay className="fixed inset-0 z-50 bg-black/55 flex items-center justify-center p-4"><div className="bg-white rounded-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto"><div className="p-5 border-b"><h3 className="font-black">Salvar nova versão do DRE</h3><p className="text-xs text-gray-500">Valores manuais não alteram as transações; eles criam uma versão auditável desta competência.</p></div><div className="p-5 grid grid-cols-1 sm:grid-cols-2 gap-4">{editableAccounts.map(([code, label]) => <label key={code} className="text-xs font-bold">{code} — {label}<input inputMode="decimal" value={manualValues[code] || ''} onChange={(event) => setManualValues((current) => ({ ...current, [code]: event.target.value }))} className="mt-1 w-full border rounded-lg px-3 py-2" /><span className="text-[10px] font-normal text-gray-500">{['2','4','6','7','10','14'].includes(code) ? 'Informe o valor positivo; será tratado como dedução/despesa.' : 'Pode ser positivo ou negativo.'}</span></label>)}<label className="sm:col-span-2 text-xs font-bold">Observações da versão<textarea rows={3} value={versionNote} onChange={(event) => setVersionNote(event.target.value)} className="mt-1 w-full border rounded-lg p-3" /></label></div><div className="p-5 border-t flex justify-end gap-2"><button onClick={() => setEditing(false)} className="px-4 py-2 text-xs font-bold">Cancelar</button><button onClick={saveVersion} className="px-5 py-2 bg-[#131b2e] text-white rounded-lg text-xs font-bold">Salvar nova versão</button></div></div></ModalOverlay>}
    </div>
  );
};
